...
async function sendSMS(fromDID, toDID, message) {
  ...
  const body = JSON.stringify({ channel: 'SMS', from: fromDID, to: toDID, content: { text: message } });
  ...
  async function sendOne(contact, idx) {
    ...
    if (r.status === 200) {
      ...
      try {
        ...
        const guid = (d.data && d.data.results && d.data.results[0] && d.data.results[0].messageId) ||
                     (d.data && d.data.batchId) || ('out_' + Date.now() + '_' + Math.random());
        await pool.query('INSERT INTO msgs (guid,frm,tod,msg,dir,stat,cname) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (guid) DO NOTHING',
          [guid, fromDID, toDID, msg, 'outbound', 'SENT', blast.name]);
      } catch(e) {}
    } else { failed++; }
  }
...
app.post('/api/send', async (req, res) => {
  try {
    ...
    console.log('Send response:', r.status, r.body);
    const data = JSON.parse(r.body);
    if (r.status === 200) {
      // New endpoint returns statusCode:200, data.batchId, data.results[0].messageId
      const guid = (data.data && data.data.results && data.data.results[0] && data.data.results[0].messageId) || 
                   (data.data && data.data.batchId) ||
                   (data.results && data.results[0] && data.results[0].messageId) ||
                   (data.batchId) || ('out_' + Date.now());
      console.log('Storing guid:', guid);
      await pool.query('INSERT INTO msgs (guid,frm,tod,msg,dir,stat,cname) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (guid) DO NOTHING',
        [guid, from_did, to_did, message, 'outbound', 'SENT', campaign_name || '']);
...
app.post('/webhook/dlr', async (req, res) => {
  try {
    ...
    console.log('DLR received:', JSON.stringify(b));
    const guid = b.guid || b.sms_guid || b.message_id || b.id;
    const sc = String(b.status || b.send_status || b.delivery_status || '').toUpperCase();
    let stat = 'SENT';
    if (sc === 'DELIVRD' || sc === 'DELIVERED' || sc.includes('DELIVER') || sc === '200') stat = 'DELIVERED';
    else if (sc.includes('FAIL') || sc === '400' || sc === '500') stat = 'FAILED';
    else if (sc.includes('UNDELIVER')) stat = 'UNDELIVERED';
    console.log('DLR guid:', guid, 'status:', stat, 'raw status:', sc);
    if (guid) {
      const r = await pool.query('UPDATE msgs SET stat=$1 WHERE guid=$2 RETURNING id', [stat, guid]);
      console.log('DLR updated rows:', r.rowCount);
    }
    res.json({ ok: true });
...
// Export inbound messages as CSV
app.get('/api/export/inbound', async (req, res) => {
  try {
    const { date } = req.query;
    let q = "SELECT frm as from_number, tod as to_number, msg as message, stat as status, ts as timestamp FROM msgs WHERE dir='inbound'";
    const params = [];
    if (date) {
      q += ' AND ts::date = $1';
      params.push(date);
    } else {
      q += " AND ts >= NOW() - INTERVAL '24 hours'";
    }
    q += ' ORDER BY ts DESC';
    const r = await pool.query(q, params);
    const csv = ['From,To,Message,Status,Timestamp',
      ...r.rows.map(row => `${row.from_number},${row.to_number},"${(row.message||'').replace(/"/g,'""')}",${row.status},${row.timestamp}`)
    ].join('\n');
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', 'attachment; filename="replies-' + (date || 'today') + '.csv"');
    res.send(csv);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
