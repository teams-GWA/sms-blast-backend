...
async function initScheduler() {
  ...
  `);
  // Check for pending blasts every minute
...
async function checkScheduledBlasts() {
  try {
    ...
    for (const blast of r.rows) {
      ...
      fireBlast(blast).then(async () => {
        await pool.query("UPDATE scheduled_blasts SET status='sent' WHERE id=$1", [blast.id]);
      }).catch(async (e) => {
...
async function fireBlast(blast) {
  ...
  async function sendOne(contact, idx) {
    ...
    const toRaw = contact.Phone1 || contact.phone || Object.values(contact)[1] || '';
    let toDID = String(toRaw).replace(/\D/g, '');
    if (toDID.length === 11 && toDID[0] === '1') toDID = toDID.slice(1);
    if (!toDID) { failed++; return; }
    const msg = blast.msg.replace(/\{([\w ]+)\}/g, (_, k) => contact[k.trim()] || '');
    const body = JSON.stringify({ from_did: fromDID, to_did: toDID, message: msg });
      ...
      const req = https.request(options, (res) => {
        ...
        res.on('end', async () => {
          if (res.statusCode === 200) {
            ...
            await pool.query('INSERT INTO msgs (guid,frm,tod,msg,dir,stat,cname) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (guid) DO NOTHING',
              [d.guid || ('out_' + Date.now()), fromDID, toDID, msg, 'outbound', 'SENT', blast.name]).catch(()=>{});
          } else { failed++; }
          resolve();
        });
      });
  ...
  await pool.query('INSERT INTO campaigns (name,total,sent,failed,msg) VALUES ($1,$2,$3,$4,$5)',
    [blast.name, contacts.length, sent, failed, blast.msg]).catch(()=>{});
  console.log('Blast complete:', blast.name, sent, 'sent', failed, 'failed');
}

// Schedule a blast
app.post('/api/schedule', async (req, res) => {
  try {
    const { name, fire_at, dids, contacts, msg, concur, batch_delay } = req.body;
    if (!fire_at || !dids || !contacts || !msg) return res.status(400).json({ error: 'Missing fields' });
    const r = await pool.query(
      'INSERT INTO scheduled_blasts (name,fire_at,dids,contacts,msg,concur,batch_delay) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id',
      [name || 'Scheduled Blast', new Date(fire_at).toISOString(), JSON.stringify(dids), JSON.stringify(contacts), msg, concur || 50, batch_delay || 50]
    );
...
app.get('/api/schedule', async (req, res) => {
  try {
    const r = await pool.query("SELECT id,name,fire_at,status,created_at FROM scheduled_blasts ORDER BY fire_at ASC");
    res.json(r.rows);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
